...
class traj_3D_obj {
  private:
    ...
    dependent_swap<float> **w;       //Velocity in the z direction.

    dependent_swap<float> *sfc;       //Surface pressure

    simple<float> *xgrid;          //Gridding in the x direction
    simple<float> *ygrid;          //Gridding in the y direction
    simple<float> *zgrid;          //Gridding in the y direction
  ...
  public:
    ...
    //traj_int_obj();
    traj_3D_obj(char **filename, simple<float> * z);
    virtual ~traj_3D_obj();

    void set_page_size(sub_1d_type page_size);

    virtual void integrate(float p0[3], float pf[3], double tstart, double tstep, long nt);

    //integrates along pre-determined time values:
    float ** int_tarray(float p0[3], time_class t0, time_class *tarr, long nt);
    ...
    int get_result(time_class date, float p[3]);
...
inline interpol_index traj_3D_obj::get_tind(time_class t) {
