...
Substituting this into the eigenvalue equation in (\ref{right_SV_eigenproblem}):
\begin{equation}
	R^T \cdot R \cdot \sum_i c_i \vec v_i = \sum_i c_i s_i^2 \vec v_i
\end{equation}
and multiplying with $\vec q$:
\begin{eqnarray}
...
%Therefore:
%\begin{equation}
%	\forall q~ (\vec q \cdot R^T \cdot R \cdot \vec q \le \vec q \cdot \vec q) \iff \forall s ~(R^T \cdot R \cdot \vec v = s^2 \vec v \land s \le 1)
%\end{equation}
