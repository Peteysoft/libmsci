...
the gradient of $q$:
\begin{eqnarray}
	\nabla_{\vec x} q & = & \nabla_{\vec x} \Phi^{-1} \int_V 
	\delta^\prime \left [\vec x_0 - \Phi^{-1}(\vec x) \right ] q_0 (\vec x_0)
	\mathrm d \vec x_0 \\
	& = & H^\prime \cdot \nabla_{\vec x_0} q_0
\end{eqnarray}
...
there will be an inverse transport map, $Q^{-1}$, from which we can derive
...
where
\begin{equation}
	k = \frac{\int_V Q_0 (\vec x^\prime) q(\vec x^\prime) \mathrm d \vec x^\prime}
	{s_{h^\prime1} \nabla_{\vec x_0} q_0 \cdot \vec h_0^\prime}
\end{equation}
is a constant of proportionality.
If diffusion is present, this relationship will only be approximate and will
grow weaker with time.
