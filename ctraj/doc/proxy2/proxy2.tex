...
Here we introduce a new method more appropriate for shorter-lived tracers
because it works on shorter time scales.
The method is summarized as follows: 
the tracer dynamics are represented as a matrix, which we term the 
\italics{transport map} because it linearly maps the intial tracer configuration to
the final tracer configuration.
The transport map is decomposed using singular value decomposition (SVD),
also called principal component analysis (PCA), hence the name of the method,
and the largest principal components are then fitted to the measurements.
Because the transport map is integrated over only a short time scale and 
because it represents all possible tracer configurations, it should be better
able to 
...
\begin{equation}
	...
	\label{SVD}
\end{equation}
where $S$ is a diagonal matrix of \italics{singular values},
...
$\lbrace \vect v^{(i)} \rbrace$, which, when the tracer dynamics are applied,
map onto a set of orthogonal final conditions, 
$\lbrace \vect u^{(i)} \rbrace$,
...
The singular values are all positive and are normally arranged from largest to smallest:
\begin{equation}
	s_i \ge s_{i-1}
\end{equation}
...
the \italics{measurement window}.
...
problem:
\begin{equation}
	\min_{\vect c} \sum_i \left [ \sum_{j=0}^N c_j (\vect w_i \cdot \vect \Phi)^j - m_i \right ]^2
\end{equation}
...
We linearize this about $\vect r$ using the \italics{tangent vector},
$\nabla_{\vect r} f$:
\begin{eqnarray}
...
Define the \italics{tangent model}, $H$, such that:
\begin{equation} 
...
\begin{equation}
	...
	\label{sv_lt_one}
\end{equation}
thus the Lyapunov exponents in turn will be less-than-or-equal to zero.
...
(no sources or sinks) started with initial conditions $\vect q_0=\vect q(t_0)$.
...
Figure \ref{sample_SV}a. shows the first right singular vector, $\vect u_1$,
while Figure \ref{sample_SV}b. shows the corresponding left singular vector,
$\vect v_1$;
Figure \ref{sample_SV}c. shows the first second singular vector, $\vect u_2$,
while Figure \ref{sample_SV}b. shows the corresponding left singular vector,
$\vect v_2$
and so on.
...
a factor of only $2/\pi+1/2\approx0.7$
...
An example of a reconstructed ozone field are shown in Figure \ref{sample_O3_field}
...
measurements.
It would be instructive to test the method using an instrument that samples
more broadly using more principal components.
...
into account all possible configurations of a passive tracer, operates over
