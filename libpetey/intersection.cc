
#include "peteys_template_lib.h"
#include "full_util.h"

namespace libpetey {

  template <typename scalar>
  int compare_vectors(scalar *v1, scalar *v2, int n) {
    for (int i=0; i<n; i++) {
      if (v1[i]<v2[i]) return -1;
      if (v1[i]>v2[i]) return 1;
    }
    return 0;
  }

  template <typename scalar>
  int intersect_vectors(scalar ***v, 		//list of lists of vectors
		  int *n, 			//number of elements in each list
		  int n0, 			//number of lists
		  int D,			//dimension of each vector
		  scalar **result,		//intersection
		  int **ind) {			//indexes into result
    scalar **all;
    long *sind;
    int k;
    int zone;
    int nt[n0+1];

    //total number of vectors:
    nt[0]=0;
    for (int i=0; i<n0; i++) nt[i+1]=nt[i]+n[i];

    //stick them all in a single array:
    //(and initialize indices while we're at it...)
    all=new scalar *[nt];
    for (int i=0; i<n0; i++) {
      for (int j=0; j<n[i]; j++) all[j+nt[i]]=v[i][j];
    }

    //sort the array:
    sind=heapsort(result, nt[n0], &compare_vectors<scalar>);

    //fill result array only with non-duplicates;
    //fill index arrays with location of these
    k=0;
    for (int i=0; i<nt[n0]-1; i++) {
      zone=bin_search(nt, n0+1, sind[i], -1);
      ind[zone][sind[i]-nt[zone]]=k;
      if (compare_vectors(all[sind[i]], all[sind[i]], D)!=0) {
        result[k]=all[sind[i]];
	k++;
      }
    }
    //last one is always in there:
    result[k]=all[sind[nt[n0]-1]];
    zone=bin_search(nt, n0+1, sind[i], -1);
    ind[zone][sind[nt[n0]]-nt[zone]]=k;

    return k;

  }

  template <typename tp>
  int32_t intersection(tp *l1, int32_t n1, tp *l2, int32_t n2, tp *lr) {
    long *s1, *s2;
    int32_t i, j, k;

    s1=heapsort(l1, n1);
    s2=heapsort(l2, n2);

    i=0;
    j=0;
    k=0;
    do {
      if (l1[s1[i]] < l2[s2[i]]) {
        i++;
      } else if (l1[s1[i]] > l2[s2[i]]) {
        j++;
      } else {
        lr[k]=l1[i];
	i++;
	j++;
	k++;
      }
    } while (i<n1 && j<n2);

    return k;
  }

  //returns 0 on success:
  template <typename scalar>
  int test_intersect_vectors(int nv,		//number of vectors
		  int D,			//dimension of each vector
		  int nss,			//number of sub sets
		  float frac) {			//approx. fraction for each ss
    scalar **v;			//all the vectors
    scalar ***ss;		//subsets
    int n[nss];			//number in each subset

    int **ind;			//list of indices
    scalar **result;		//intersection
    int nres;
    int foundflag;

    ran_init();

    //create test matrices:
    v=allocate_matrix<scalar>(nv, D);
    for (int i=0; i<nv; i++) {
      for (int j=0; j<D; j++) {
        v[i][j]=ranu();
      }
    }

    for (int i=0; i<nss; i++) {
      ss[i]=allocate_matrix<scalar>(nv, D);
      n[i]=0;
      for (int j=0; j<nv; j++) {
        if (ranu() < frac) {
          for (int k=0; k<D; k++) ss[i][n[i]][k]=v[i][j];
	  n[i]++;
	}
      }
    }

    ran_end();
    delete_matrix(v);

    //find the intersection:
    result=allocate_matrix<scalar>(nv, D);
    ind=allocate_matrix<int>(nss, nv);
    nres=intersect_vectors(ss, n, nss, D, result, ind);

    //indices must be found in the sub-sets:
    for (int i=0; i<nss; i++) {
      for (int j=0; j<n[i]; j++) {
        foundflag=0;
        if (compare_vectors(result[ind[j]], ss[i][j])!=0) {
          fprintf(stderr, "test_intersect_vectors: incorrect index, ind[%d]=%d\n", j, ind[j]);
          goto fail;
	}
      }
    }

    //result must be sorted:
    for (int i=1; i<nv; i++) {
      if (compare_vectors(result[i-1], result[i])>=0) {
        fprintf(stderr, "test_intersect_vectors: result vector at %d duplicated/not sorted\n", i);
        goto fail;
      }
    }

    delete_matrix(result);
    delete_matrix(ind);
    for (int i=0; i<nss; i++) delete_matrix(ss[i]);

    return 0;

    fail:
      for (int i=0; i<nss; i++) {
        print_matrix(stdout, ss[i]);
	printf("\n");
	for (int j=0; j<n; j++) {
          printf("%d ", ind[i][j]);
	}
	printf("\n");
      }
      print_matrix(stdout, result);


    delete_matrix(result);
    delete_matrix(ind);
    for (int i=0; i<nss; i++) delete_matrix(ss[i]);

    return -1;
  }

}
