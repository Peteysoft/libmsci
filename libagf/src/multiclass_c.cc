...
namespace libagf {
  ...
  real multiclass_c<real, cls_t>::ret(real *x, real &err) {
    ...
    cls_t l1, l2;
    int ng=0;			//number of decision values
    //legth of gradient vector -- borders classifier pre-calculates lengths
    //of all the gradient vectors; 
    //once returned must be determined by index of closest border sample:
    real *gd=raw+this->ncls;
