...
-0 & Read from stdin/trial run/output raw data/omit final processing.\\
-1 & Write to stdout.\\
...
-Q & Algorithm selection:\\
  ...
  & \verb/  /non-hierarchical multi-class classification:\\
	& \verb/    / 0=Lawson \& Hanson constrained least squares (default);\\
  ...
  & \verb/    / 4=least squares with normalization constraint 1;\\
  & \verb/    / 5=least squares with normalization constraint 2;\\
  & \verb/    / 6=least squares with renormalization;\\
  & \verb/    / 7=constrained inverse (may not be optimal);\\
  & \verb/    / 8=voting from pdf with renormalization;\\
  & \verb/    / 9=one versus the rest;\\
	& \verb/    /10=iterative method by Zadrozny;\\
  & \verb/    /11=constrained inverse 2 (may be extremely inefficient);\\
\end{tabular}
