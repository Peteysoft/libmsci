...
Pixels that always take on the same value have been removed.

The ``seismic'' dataset deals with vehicle classification from seismic data
\citep{Duarte_Hu2004}.
The ``mushroom'' dataset classifies wild mushrooms
into poisonous and non-poisonous types based on their physical characteristics \citep{Isa_etal1988}.
...
\citep{Lichman2003} and the third source was the author.
...
It failed on three of the datasets, succeeded on eight and generated
mixed results for the remaining six datasets.
By fail, we mean either it was significantly slower than the SVM 
or it produced an unacceptable reduction in accuracy.
By mixed, we mean that either the algorithm failed to produce a significant
speed increase over the SVM
or it produced a small but signficant reduction in accuracy.
By succeed we mean that it both significantly improved the speed of the SVM
and produced an acceptable reduction in accuracy.
We will make the meaning of an acceptable reduction in accuracy more precise
in the next section.

To save space, only six datasets will be discussed here.
Two for which the algorithm failed, two for which it produced mixed results
and two for which it failed.
These datasets are summarized in Table \ref{summary} and are described in
more detail below.
Three of the datasets come from the LIBSVM project page, two from the
University of California at Irvine (UCI) machine learning repository,
and one is from the author.
The datasets from the LIBSVM project page have both test and training data in which case they were used
as defined. If all instances have been lumped together, then the data was split
randomly into both test and training and test classifications repeated over 10 trials.
...
That is, during protein creation, part of the sequence is spliced out, with
the section kept being the intron and that spliced out being the extron.
The data was taken from the LIBSVM repository and has been reprocessed so that
the 60 DNA base values were transformed to 180 binary attributes.
\citep{Michie_etal1994}
...
\subsection{Pendigits dataset}

The pendigits dataset is similar to the letter dataset except now we are 
classify numbers instead of letters \citep{Alimoglu1996}.
This dataset was obtained from the LIBSVM repository.
...
The shuttle dataset is part of the Statlog project \citep{King_etal1995,Michie_etal1994}, comes originally
from NASA and was taken from an actual space shuttle flight.
The classes describe actions to be taken at different flight configurations.
...
\subsection{RNA coding dataset}

This datasets deals with detection of non-coding RNA sequences 
\citep{Uzilov_etal2006}.
