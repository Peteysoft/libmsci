...
binary classification problems in which the two classes are both closely 
...
Because of the versatility, generality and simplicity of such methods there has been recent renewed interest 
\citep{Bagirov2005, Kostin2006, Gai_Zhang2010, Webb2012, Wang_Saligrama2013, Pavlidis_etal2016}.

A linear classifier typically takes the form:
\begin{equation}
...
feature space to figure out which linear decision boundary to use for a given 
test point.
...
If the class domains are simply connected, the partitions will also tend to
be pyramidal in shape, with axes of the pyramids roughly perpendicular to
the decision border.
...
decision boundary through global optimization \citep{Bagirov2005, Wang_Saligrama2013} or through
methods that are more local \citep{Gai_Zhang2010}.
We can also distinguish between simultaneous training of the decision surfaces 
versus methods that are more piece-meal \citep{Herman_Yeung1992, Kostin2006}.
Obviously, global, simultaneous optimization will be more accurate than methods
that are local and piece-meal but also much more computationally expensive.
...
Then there is the issue of separability. Many of the current crop of 
methods seem to be designed with disjoint classes in mind \citep{Herman_Yeung1992}, for instance
\citet{Gai_Zhang2010} who stick the hyper-plane borders between pairs of 
opposing classes.
...
because it is not actually a stand-alone method and requires estimates
of the conditional probabilities.
It is used to improve the time performance of kernel methods while 
maintaining, in all but a few cases, most of the accuracy.
...
Section \ref{results} outlines the results for 17 case studies
...
simple matrix solver is more than sufficient.
...
Sometimes the speed increase is so great that even the AGF-borders method will provide an
...
at which point the AGF-borders method starts to train faster than LIBSVM borders.
...
Neither SVM nor the borders method return an integer that represents the class,
but rather a continuum value (or more precisely a limited-precision floating point 
value that approximates this real number) whose sign determines the class.
Thus a single measure of classification skill is not a complete representation
of the classifiers' accuracy since 
it's possible to recalibrate the results by choosing a different threshold.
This can be done either to maximize a particular skill score or to account for
different costs for the different types of error and different rewards for
the different correct guesses.

Consider the confusion matrices for the splice problem for both SVM
and SVM-borders classifiers shown in Table \ref{confusion}.
...
The method succeeded for seven of the datasets and failed for three while it
broke even for six.
Not a stellar performance but certainly worthwhile to try for operational retrievals
...
where it might fail.
Since it was designed with continuum features data in mind, it tends to work poorly
with integer or categorical data.
...
This could be mitigated somewhat by using a multi-class classification method
