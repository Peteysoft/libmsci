...
matrix} and $p_i = p(i | \vec x)$ is the conditional probability of the $i$th
class.
In vector notation:
\begin{equation}
...
\begin{eqnarray}
	\forall i & \left | \sum_{j=0}^n a_{ij} \right | < n & \label{restriction}\\\nonumber
	\forall j & \left | \sum_{i=0}^m a_{ij} \right | < m & 
\end{eqnarray}
...
The first three rows (\ref{harmonic}) comprise a Walsh-Hadamard code \citep{Arora_Barak2009}.
...
\end{eqnnon}
Note that in this ordering, there will be $m$ $i$'s indexing the same $\vec x$ 
and the same $y$.
