...
\input{orthogonal_results2018_08_13.tex}
\end{table}
...
The method was compared with random error correcting codes using an equivalent
number of codes (the number binary classifiers, or columns in the matrix, $n$, is kept constant).
These were solved using a constrained linear least squares method based
on the Karesh-Kuhn-Tucker conditions \citep{Hanson_Lawson1993}.
...
For most of the datasets, orthogonal coding matrices provide a small but
...
Using orthogonal coding matrices appears to be slightly faster as well,
although this difference is less significant.
The speed difference results because the solution method is much simpler.
Since actually solving for the conditional probabilities is such a minor
component of multi-class classification, the difference is small.
The rest of the calculation should be roughly equivalent speed-wise.
...
the method is very simple and elegance and
