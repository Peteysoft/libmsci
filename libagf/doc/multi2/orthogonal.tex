...
\begin{itemize}
	...
	\item re-arranging the columns (but not the rows) of $A$ makes an equivalent $A$: $A T_{ij}^T$ is equivalent to $A$ where $T_{ij}$ is the elementary matrix which exchanges rows $i$ and $j$
\end{itemize}
