...
void read_wind_file(char *filename) {
  ...
  varname.print();
  printf(": type %d\n", type);
  u=new dependent<float>(lon, lat, tgrid);
  printf("%d elements\n", u->nel());
  u->read(lun);
  ...
  varname.print();
  printf(": type %d\n", type);
  v=new dependent<float>(lon, lat, tgrid);
  v->read(lun);
...
int main(int argc, char *argv[]) {
  ...
  do {
    ...
    intind[0]=lon->interp(x);
    intind[1]=lat->interp(y);
    intind[2]=tgrid->interp(t);
    
    u->interpol_old(ui, intind);
    v->interpol_old(vi, intind);
